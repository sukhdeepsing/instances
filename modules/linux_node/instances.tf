...
provisioner "remote-exec" {
     connection {
        ...
        user = "ubuntu"
        agent = false
        private_key = file("../keys/student.1-vm-key")
      }
    inline = [
...
resource "null_resource" "install_package" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u root -i ./tf_ansible_${var.install_package}_inventory.ini ../ansible-playbooks/${var.playbook_name} --private-key '../keys/student.1-vm-key'"
}
  provisioner "local-exec" {
    command = "sudo apt-get install openjdk-11-jdk -y"
}
}
